...
namespace MvvmBurgerApp.ViewModels
{
    public class BurgerViewModel : INotifyPropertyChanged
    {
        ...
        public ICommand UpTCountCommand { protected set; get; }
        public ICommand DownTCountCommand { protected set; get; }
        public ICommand UpICountCommand { protected set; get; }
        public ICommand DownICountCommand { protected set; get; }
        ...
        public BurgerViewModel()
        {
            ...
            UpTCountCommand = new Command(UpTCount);
            DownTCountCommand = new Command(DownTCount);
            UpICountCommand = new Command(UpICount);
            DownICountCommand = new Command(DownICount);
            Burger = new Burger();
            ...
            Burger.Luck = new Additionally("Лук",5,0);
            Burger.Ogurce = new Additionally("Огурцы", 15, 0);
            //{
        ...
        private void RecalTotal()
        {
            if (Count > 0)
            {
                Total = Count * Price + SalatCount * Burger.Salat.Price + TomatCount * Burger.Tomat.Price + LuckCount * Burger.Luck.Price + OgurceCount * Burger.Ogurce.Price;
            }
        }
        ...
        private void UpTCount()
        {
                LuckCount += 1;
            RecalTotal();
        }
        private void DownTCount()
        {
            if (LuckCount > 0)
                LuckCount -= 1;
            RecalTotal();
        }
        private void UpICount()
        {
            OgurceCount += 1;
            RecalTotal();
        }
        private void DownICount()
        {
            if (OgurceCount > 0)
                OgurceCount -= 1;
            RecalTotal();
        ...
        {
            get { return Burger.Luck.Count; }
            set
            {
                if (Burger.Luck.Count != value)
                {
                    Burger.Luck.Count = value;
                    OnPropertyChanged("LuckCount");
                }
            }
        }
        public int OgurceCount
        {
            get { return Burger.Ogurce.Count; }
            set
            {
                if (Burger.Ogurce.Count != value)
                {
                    Burger.Ogurce.Count = value;
                    OnPropertyChanged("OgurceCount");
                }
            }
        }
